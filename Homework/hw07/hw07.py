...
class VendingMachine:
    ...
    def deposit(self, amount):
        if self.stock == 0:
            ...
            self.balance = 0
            return 'Machine is out of stock. Here is your ${0}.'. format(temp_balance + amount)
        else:
            self.balance += amount
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            temp_balance2 = self.balance
            self.balance = 0
            self.stock -= 1
            return 'Here is your {0} and ${1} change.'.format(self.product, temp_balance2 - self.price)
        elif self.balance == self.price:
            self.balance = 0
            self.stock -= 1
...
class MissManners:
    ...
    def ask(self, message, *args):
        magic_word = 'please '
        method = message[len(magic_word):]
        if not message.startswith(magic_word):
            return 'You must learn to say please first.'
        elif hasattr(self.obj, method):
            method_call = getattr(self.obj, method)
