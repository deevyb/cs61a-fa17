...
CREATE TABLE schedule AS
  WITH trips(route, destination, num_flights, cost) AS (
    SELECT departure || ", " || arrival, arrival, 1, price FROM flights WHERE departure = "SFO" UNION
    SELECT route || ", " || arrival, arrival, num_flights+1, cost+price FROM trips, flights WHERE destination = departure AND num_flights < 2
  )
  SELECT route, cost FROM trips WHERE destination = "PDX" ORDER BY cost;

CREATE TABLE number_of_options AS
  SELECT COUNT(DISTINCT meat) FROM main_course;

CREATE TABLE calories AS
  SELECT COUNT(*) FROM main_course AS mc, pies AS p WHERE mc.calories + p.calories < 2500;

CREATE TABLE healthiest_meats AS
  SELECT meat, MIN(mc.calories+p.calories) FROM main_course AS mc, pies AS p GROUP BY meat HAVING MAX(mc.calories+p.calories) < 3000;
...
CREATE TABLE shopping_list AS
  WITH helper(item, price) AS (
    SELECT name, MIN(MSRP/rating) FROM products GROUP BY category
  )
  SELECT h.item, lp.store FROM helper AS h, lowest_prices AS lp WHERE h.item = lp.item;

CREATE TABLE total_bandwidth AS
  SELECT SUM(Mbs) FROM stores AS s, shopping_list AS sl WHERE s.store = sl.store;
