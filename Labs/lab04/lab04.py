...
def closer_city(lat, lon, city1, city2):
    ...
    distance1 = distance(test_city, city1)
    distance2 = distance(test_city, city2)
...
def put_piece(board, max_rows, column, player):
    ...
    while counter != 0:
        counter -= 1
        if get_piece(board, counter, column) == '-':
            new_row = replace_elem(board[counter], column, player)
            board = replace_elem(board, counter, new_row)
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    if col >= 0 and get_piece(board, 0, col) == '-':
