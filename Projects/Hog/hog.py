...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    score_as_digits = [int(x) for x in str(score)]
    return 1+max(score_as_digits)
    # END PROBLEM 2
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if change > previous_high:
            ...
            return announce_highest(who, change, current_score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    num_dice_rolled = 1
    result = make_averaged(roll_dice, num_samples)
    current_highest = 0
    best_roll_count = 0

    while num_dice_rolled <= 10:
        if result(num_dice_rolled, dice) > current_highest:
            current_highest = result(num_dice_rolled, dice)
            best_roll_count = num_dice_rolled
        num_dice_rolled += 1
