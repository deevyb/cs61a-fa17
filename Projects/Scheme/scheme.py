...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest
        return operator.eval_call(operands, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('improper number of argument')
        else:
            while formals:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        new_operands = operands.map(lambda operand: scheme_eval(operand, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        unevaluated = scheme_apply(self, new_operands.expr, env)
        return scheme_eval(complete_eval(unevaluated), env)
        # END Problem 21
...
def make_let_frame(bindings, env):
    ...
    while not (bindings is nil):
        ...
        first, second = bindings.first.first, bindings.first.second.first
        formals, vals = Pair(first, formals), Pair(scheme_eval(second, env), vals)
        bindings = bindings.second
    check_formals(formals)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            check_procedure(operator)
            operands = rest
            result = operator.eval_call(operands, env)
            # END PROBLEM 20
    return result
